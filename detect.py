...
#If there's aGPU available, put the model on GPU
if CUDA:
    model.cuda()
...
# List conating dimensions of original images
...
for i, batch in enumerate(im_batches):
    ...
    if type(prediction) == int:
        for im_num, image in enumerate(imlist[i*batch_size:min((i+1)*batch_size, len(imlist))]):
            ...
            print("----------------------------------------------------------")
        continue
    prediction[:,0] += i*batch_size # Transform the attribute from index in batch to in imlist
...
# Whether ther has been a single detection has been made or not
...
# Modifies the images inside loded_ims inplace

list(map(lambda x: write(x, loaded_ims),output))

# List of addresses to which we will ave our detection images to

det_names = pd.Series(imlist).apply(lambda x: "{}/det_{}".format(args.det,x.split("/")[-1]))

# Write the images with detection to the address in det_names
...
# To print the summary, to compare the different hyperparameters
